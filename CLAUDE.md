...
> Última atualização: 2026-08-20 — **e-mail transacional funcionando de verdade em produção, pela primeira vez.** Sessão longa em cima do que ficou pendente de 2026-08-19 (fix do 500 em "Definir senha", "Perfil de acesso" automático, documento/áudio no fórum de comentários com comentário podendo ser só-mídia — tudo isso já promovido pra produção nesta sessão, sem pendência). O grosso do dia foi descobrir e resolver por que o SMTP não funcionava: (1) `naoresponda@fnp.org.br` criado no Workspace, mas o primeiro teste real deu `Network is unreachable` — container sem rota IPv6, corrigido forçando IPv4 em `settings.py`; (2) segundo teste deu `Connection timed out` — investigação em camadas (`ufw` e Cloud Firewall da DO liberam tudo) revelou que a **própria DigitalOcean bloqueia por padrão as portas de SMTP de saída (587/465)** em toda conta nova; chamado de suporte aberto, mas **não esperamos a resposta** — (3) implementado `GmailApiEmailBackend` (`apps/legislativo/email_backends.py`), que manda e-mail via Gmail API sobre HTTPS (porta sempre liberada), autenticando com Service Account + Domain-wide Delegation autorizada no Workspace, testado local e depois em produção de verdade (`enviado sem excecao`, e-mail confirmado chegando). (4) No caminho, achado que **todo** e-mail transacional mostrava "example.com" no assunto/corpo — `django.contrib.sites` nunca tinha sido configurado com o domínio real; corrigido via migration + pequeno ajuste de gramática no template-base do allauth. Usuário autorizou acesso SSH direto ao droplet nesta sessão (chave já configurada localmente) — usado pra configurar `.env` sem nunca expor a credencial no chat, e pra rodar os deploys finais. `next` → `main` → produção → droplet confirmado saudável em cada etapa. **Pendência de segurança**: senha do `doadmin` exposta em chat de novo (query colada sem querer) — já rotacionada pelo usuário na mesma sessão. Ver seções datadas mais abaixo pro detalhe de cada item. Sessão anterior (2026-08-13/14) foi a auditoria de UX mobile + fixes de produção — ver seções datadas mais abaixo.
